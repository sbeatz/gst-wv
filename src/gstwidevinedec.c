...
GST_DEBUG_CATEGORY_STATIC (gst_widevine_decrypt_debug_category);
#define GST_CAT_DEFAULT gst_widevine_decrypt_debug_category
...
/* prototypes */
static void gst_widevine_decrypt_dispose (GObject * object);
static void gst_widevine_decrypt_finalize (GObject * object);

static gboolean gst_widevine_decrypt_start (GstBaseTransform * trans);
static gboolean gst_widevine_decrypt_stop (GstBaseTransform * trans);
static gboolean gst_widevine_decrypt_append_if_not_duplicate(GstCaps *dest, GstStructure *new_struct);
static GstCaps *gst_widevine_decrypt_transform_caps (GstBaseTransform * base,
...
static GstStaticPadTemplate gst_widevine_decrypt_sink_template =
    GST_STATIC_PAD_TEMPLATE ("sink",
    GST_PAD_SINK,
    GST_PAD_ALWAYS,
    GST_STATIC_CAPS
    ("application/x-cenc, original-media-type=(string)video/mp4, protection-system=(string)69f908af-4816-46ea-910c-cd5dcccb0a3a; "
        "application/x-cenc, original-media-type=(string)video/mp4, protection-system=(string)5e629af5-38da-4063-8977-97ffbd9902d4; "
        "application/x-cenc, original-media-type=(string)audio/mp4, protection-system=(string)69f908af-4816-46ea-910c-cd5dcccb0a3a; "
        "application/x-cenc, original-media-type=(string)audio/mp4, protection-system=(string)5e629af5-38da-4063-8977-97ffbd9902d4")
    );

static GstStaticPadTemplate gst_widevine_decrypt_src_template =
    GST_STATIC_PAD_TEMPLATE ("src",
    GST_PAD_SRC,
    GST_PAD_ALWAYS,
    GST_STATIC_CAPS ("video/mp4; audio/mp4")
    );
...
gst_widevine_decrypt_class_init (GstWidevineDecryptClass * klass)
{
  ...
  gst_element_class_add_pad_template (element_class,
      gst_static_pad_template_get (&gst_widevine_decrypt_sink_template));
  gst_element_class_add_pad_template (element_class,
  ...
  base_transform_class->start = GST_DEBUG_FUNCPTR (gst_widevine_decrypt_start);
  base_transform_class->stop = GST_DEBUG_FUNCPTR (gst_widevine_decrypt_stop);
  base_transform_class->transform_ip =
      GST_DEBUG_FUNCPTR (gst_widevine_decrypt_transform_ip);
  base_transform_class->transform_caps =
      GST_DEBUG_FUNCPTR (gst_widevine_decrypt_transform_caps);
  base_transform_class->sink_event =
      GST_DEBUG_FUNCPTR (gst_widevine_decrypt_sink_event_handler);
...
gst_widevine_decrypt_init (GstWIDEVINEDecrypt * self)
{
  ...
  self->keys = g_ptr_array_new_with_free_func (gst_widevine_keypair_destroy);
}
...
*/
static gboolean
gst_widevine_decrypt_append_if_not_duplicate(GstCaps *dest, GstStructure *new_struct)
{
...
static void
gst_widevine_remove_codec_fields (GstStructure *gs)
{
...
*/
static GstCaps *
gst_widevine_decrypt_transform_caps (GstBaseTransform * base,
    GstPadDirection direction, GstCaps * caps, GstCaps * filter)
{
  GstCaps *res = NULL;
  gint i, j;
  ...
  for (i = 0; i < gst_caps_get_size (caps); ++i) {
    ...
    if (direction == GST_PAD_SINK) {
      ...
      gst_widevine_remove_codec_fields (tmp);
...
static gchar *
gst_widevine_bytes_to_string (gconstpointer bytes, guint length)
{
...
static gchar *
gst_widevine_create_content_id (gconstpointer key_id)
{
...
gst_widevine_decrypt_get_key (GstWidevineDecrypt * self, GstBuffer * key_id)
{
  ...
  hash_string = gst_widevine_bytes_to_string (hash, SHA_DIGEST_LENGTH);
...
error:
  g_free (path);
  gst_widevine_keypair_destroy (kp);
  return NULL;
}

static gchar *
gst_widevine_create_uuid_string (gconstpointer uuid_bytes)
{
...
gst_Widevine_decrypt_lookup_key (GstWidevineDecrypt * self, GstBuffer * kid)
{
  ...
  /*
    ...
    id_string = gst_widevine_create_uuid_string (info.data);
  ...
  */
  ...
  if (!kp) {
    kp = gst_widevine_decrypt_get_key (self, kid);
  }
